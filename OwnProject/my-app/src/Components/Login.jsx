...
const inisiate={
 ...
 const Login=()=>{
  ...
  //const {isAuth,ToggleStatus}=useContext(AppContext)
  ...
  const HandleSubmit=(e)=>{
    ...
    setSubmit(true)
    if(details.name!=="" && details.email!=="" && details.password.length>4 && details.password.length<=10 ){
  ...
  const isValide=(values)=>{
     ...
     if(!values.name){
        errors.name="Username is requeried";
     }  
     if(!values.email){
        ...
        <div style={{display:"grid",width:"400px",margin:"auto",marginBottom:"50px",boxShadow: "rgba(50, 50, 93, 0.25) 0px 6px 12px -2px, rgba(0, 0, 0, 0.3) 0px 3px 7px -3px" }}>
          ...
          <form onSubmit={HandleSubmit}>
            <div style={{margin:"15px"}}>
              <Input placeholder="Enter_userName" type="text" value={details.name} name="name" onChange={HandleChange} />
            </div>
